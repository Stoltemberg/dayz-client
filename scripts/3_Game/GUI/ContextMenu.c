...
class ContextMenu extends ScriptedWidgetEventHandler
{
	...
	void ContextMenu()
	{
		m_commands = new array<CallQueueContext>;
		m_count = 0;
	}
	//--------------------------------------------------------------------------
	void ~ContextMenu()
	{
		...
		{
			m_context_menu_root_widget.Destroy();
			m_context_menu_root_widget = NULL;
		}
	}
	//--------------------------------------------------------------------------
	void Init(Widget layoutRoot)
	{
		...
		if (!m_context_menu_root_widget) return;
	...
	void Show(int x, int y)
	{
		if ( m_count == 0) return;
		if (!m_context_menu_root_widget) return;
		if (!m_context_menu_panel_widget) return;
		...
		{
			ButtonWidget menu_button = m_context_menu_root_widget.FindAnyWidget( String( "Button" + (i+1).ToString() ) );
			if (menu_button)
			{
	...
	void Hide()
	{
		if (m_context_menu_root_widget) m_context_menu_root_widget.Show(false);
		Clear();
	}
	...
	bool IsVisible()
	{
		if (!m_context_menu_root_widget) return false;
	...
	void Add(string label, Class obj, string fn_name, Param params /*= NULL*/)
	{
		if (!m_context_menu_root_widget) return;
		int count = Count();
		if (count >= ITEMS_COUNT) return;
		ButtonWidget menu_button = m_context_menu_root_widget.FindAnyWidget( String( "Button" + (count+1).ToString() ) );
	...
	void Remove(int index)
	{
		if (index < m_commands.Count())
		{
			...
			ButtonWidget menu_button = m_context_menu_root_widget.FindAnyWidget( String( "Button" + ( index + 1 ).ToString() ) );
			if (menu_button)
			{
	...
	{	
		...
		{
			CallQueueContext ctx = m_commands.Get(i);
			delete ctx;
		}
		m_commands.Clear();
