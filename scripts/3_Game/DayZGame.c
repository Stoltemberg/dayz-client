...
class DayZGame extends CGame
{
	...
	private bool 	m_CustomMapKeyHolding;
	private int		m_LastInputWatchdogTime;
	private bool	m_InputFocusPulseActive;
	...
	void DayZGame()
	{
		...
		m_CustomMapKeyHolding = false;
		m_LastInputWatchdogTime = 0;
		m_InputFocusPulseActive = false;
		...
		//ExecuteSQF("-init=playMission ['', '\d:\work\DayZ\Client\Profile\Users\Klco\missions\test_position_zero.SampleMap3']");
		
		//PlayMission("d:\\work\\DayZ\\Client\\Profile\\Users\\Klco\\missions\\test_position_zero.SampleMap3");
	...
	void OnEvent(EventType eventTypeId, Param params)
	{
		...
		case WindowsResizeEventTypeID:
			FlushStuckInputState("windows_resize");
			break;
	...
	void OnKeyPress(int key)
	{
		...
		if ((m_IsLeftAltHolding || m_IsRightAltHolding) && key == KeyCode.KC_TAB)
		{
			FlushStuckInputState("alt_tab");
		}
	...
	bool ToggleCustomMapMenu()
	{
		...
		if ( GetMissionState() != MISSION_STATE_GAME )
		{
			return false;
		}

		if ( manager.IsMenuOpen(MENU_MAP) )
		{
			Print("[CustomMap] DayZGame fechando MENU_MAP.");
			manager.Back();
			return true;
		}
		...
		if ( current_menu )
		{
			if ( current_menu.GetID() == MENU_INGAME )
			{
				manager.CloseMenu(MENU_INGAME);
			}
			else
			{
				Print("[CustomMap] DayZGame ignorou M: outro menu aberto.");
				return false;
			}
		}

		Print("[CustomMap] DayZGame abrindo MENU_MAP pelo M.");
		manager.EnterScriptedMenu(MENU_MAP, NULL);
		return true;
	}
	...
	void FlushStuckInputState(string reason)
	{
		if ( !GetGame().IsClient() )
		{
			return;
		...
		ClearKey(KeyCode.KC_RCONTROL);
		ClearKey(KeyCode.KC_LMENU);
		ClearKey(KeyCode.KC_RMENU);
		ClearKey(KeyCode.KC_TAB);
		ClearKey(KeyCode.KC_LWIN);
		ClearKey(KeyCode.KC_RWIN);

		UIManager manager = GetUIManager();
		if ( manager && manager.GetMenu() )
		{
			return;
		}
		...
		if ( input )
		{
			input.ResetGameFocus(INPUT_DEVICE_MOUSE);
			input.ResetGameFocus(INPUT_DEVICE_KEYBOARD);
			input.ChangeGameFocus(1, INPUT_DEVICE_MOUSE);
			input.ChangeGameFocus(1, INPUT_DEVICE_KEYBOARD);
			m_InputFocusPulseActive = true;
			GetCallQueue(CALL_CATEGORY_SYSTEM).Call(this, "RestoreInputAfterFlush");
		}

		Print("[InputFix] Limpando input preso: " + reason);
	}

	void RestoreInputAfterFlush()
	{
		if ( !GetGame().IsClient() )
		{
			return;
	...
	void OnUpdate(bool doSim, float timeslice)
	{
		...
		if ( m_LastInputWatchdogTime > 0 && (time_now - m_LastInputWatchdogTime) > 1500 )
		{
			FlushStuckInputState("frame_gap");
		}
		m_LastInputWatchdogTime = time_now;
