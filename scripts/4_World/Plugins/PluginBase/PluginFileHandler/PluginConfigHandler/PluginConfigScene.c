...
{
	protected const string FILE_ROOT			= "$saves:";
	...
	protected const string PARAM_OBJ_ORI		= "orientation";
	...
	// SceneRename
	//========================================
	bool SceneRename(string scene_name, string scene_name_new)
	{
	...
	// SceneDuplicate
	//========================================
	bool SceneDuplicate(string scene_name)
	{
		string file_path = GetPathScenes()+"\\"+scene_name+"."+PluginSceneManager.SCENE_SUFIX;
...
//=====================================================================================	
	...
	{
		PluginSceneManager module_scene_editor = GetPlugin(PluginSceneManager);
	...
	{
		// Save Mission Name
		CfgParamString cfg_mission = GetParamByName(PARAM_MISSION, CFG_TYPE_STRING);
		cfg_mission.SetValue(scene.GetNameMission());
				
		// Save Time
		CfgParamFloat cfg_time = GetParamByName(PARAM_TIME, CFG_TYPE_FLOAT);
		cfg_time.SetValue(scene.GetInitTime());
		
		// Save Year
		CfgParamInt cfg_year = GetParamByName(PARAM_YEAR, CFG_TYPE_INT);
		cfg_year.SetValue(scene.GetInitYear());
		
		// Save Month
		CfgParamInt cfg_month = GetParamByName(PARAM_MONTH, CFG_TYPE_INT);
		cfg_month.SetValue(scene.GetInitMonth());
		
		// Save Day
		CfgParamInt cfg_day = GetParamByName(PARAM_DAY, CFG_TYPE_INT);
		cfg_day.SetValue(scene.GetInitDay());
		
		// Save Hour
		CfgParamInt cfg_hour = GetParamByName(PARAM_HOUR, CFG_TYPE_INT);
		cfg_hour.SetValue(scene.GetInitHour());
		
		// Save Minute
		CfgParamInt cfg_minute = GetParamByName(PARAM_MINUTE, CFG_TYPE_INT);
	...
	void SaveSceneWeather(SceneData scene)
	{
		...
		CfgParamFloat cfg_overcast = GetParamByName(PARAM_OVERCAST, CFG_TYPE_FLOAT);
		cfg_overcast.SetValue(scene.GetInitOvercast());
		
		// Save Weather Rain
		CfgParamFloat cfg_rain = GetParamByName(PARAM_RAIN, CFG_TYPE_FLOAT);
		cfg_rain.SetValue(scene.GetInitRain());
				
		// Save Weather Fog
		CfgParamFloat cfg_fog = GetParamByName(PARAM_FOG, CFG_TYPE_FLOAT);
		cfg_fog.SetValue(scene.GetInitFog());
		
		// Save Weather Wind Force
		CfgParamFloat cfg_wind_force = GetParamByName(PARAM_WIND_F, CFG_TYPE_FLOAT);
		cfg_wind_force.SetValue(scene.GetInitWindForce());
	}
	
	//=============================================
	// SAVE: SaveScenePlayer (position, rotation, damage, script)
	//=============================================
	void SaveScenePlayer(SceneData scene)
	{
		array<ScenePlayer> players = scene.GetScenePlayers();
		...
		if ( players.Count() > 0 )
		{
			...
			// Save Player
			CfgParamArray cfg_param_player = GetParamByName(PARAM_PLAYER, CFG_TYPE_ARRAY);
			
			// Obj Position
			CfgParamString cfg_param_player_pos = cfg_param_player.GetValueByName(PARAM_OBJ_POS, CFG_TYPE_STRING);
			cfg_param_player_pos.SetValue(plyr.GetPositionAsString());
			
			// Obj Rotation
			CfgParamFloat cfg_param_player_rot = cfg_param_player.GetValueByName(PARAM_OBJ_ROT, CFG_TYPE_FLOAT);
			cfg_param_player_rot.SetValue(plyr.GetRotation());
		
			// Obj Damage
			CfgParamFloat cfg_param_player_dmg = cfg_param_player.GetValueByName(PARAM_OBJ_DMG, CFG_TYPE_FLOAT);
			cfg_param_player_dmg.SetValue(plyr.GetDamage());
			
			// Obj Init Script
			CfgParamString cfg_param_player_iscr = cfg_param_player.GetValueByName(PARAM_OBJ_ISCR, CFG_TYPE_STRING);
			cfg_param_player_iscr.SetValue( EncodeInitScript(plyr.GetInitScript()) );
		}
	}
	
	//=============================================
	// SAVE: SaveSceneObjects (position, rotation, damage, script, scripted links)
	//=============================================
	void SaveSceneObjects(SceneData scene)
	{
		array<SceneObject> objects = scene.GetSceneObjects();
		...
		for ( int i = 0; i < objects.Count(); ++i )
		{
			...
			// Save Scene Object
			CfgParamArray cfg_param_obj = GetParamByName(param_name, CFG_TYPE_ARRAY);
			
			// Obj Type Name
			CfgParamString cfg_param_obj_type = cfg_param_obj.GetValueByName(PARAM_OBJ_TYPE, CFG_TYPE_STRING);
			cfg_param_obj_type.SetValue(obj.GetTypeName());
			
			// Obj Position
			CfgParamString cfg_param_obj_pos = cfg_param_obj.GetValueByName(PARAM_OBJ_POS, CFG_TYPE_STRING);
			cfg_param_obj_pos.SetValue(obj.GetPositionAsString());
			
			// Obj Rotation
			CfgParamFloat cfg_param_obj_rot = cfg_param_obj.GetValueByName(PARAM_OBJ_ROT, CFG_TYPE_FLOAT);
			cfg_param_obj_rot.SetValue(obj.GetRotation());

			// Obj Orientation
			CfgParamString cfg_param_obj_ori = cfg_param_obj.GetValueByName(PARAM_OBJ_ORI, CFG_TYPE_STRING);
			cfg_param_obj_ori.SetValue(obj.GetOrientation().ToString(false));
			
			// Obj Damage
			CfgParamFloat cfg_param_obj_dmg = cfg_param_obj.GetValueByName(PARAM_OBJ_DMG, CFG_TYPE_FLOAT);
			cfg_param_obj_dmg.SetValue(obj.GetDamage());
			
			// Obj Init Script
			CfgParamString cfg_param_obj_iscr = cfg_param_obj.GetValueByName(PARAM_OBJ_ISCR, CFG_TYPE_STRING);
			cfg_param_obj_iscr.SetValue( EncodeInitScript(obj.GetInitScript()) );
			
			local EntityAI e_obj = obj.GetObject();

			local int attcount = e_obj.AttachmentsCount();
				
			if ( attcount > 0 )
			{
				local string param_name_att = param_name+"_"+PARAM_OBJ_ATT;
				CfgParamArray cfg_param_att = GetParamByName(param_name_att, CFG_TYPE_ARRAY);
				cfg_param_att.Clear();
				
				for (int att = 0; att < attcount; att++)
				{	
					EntityAI attachment = e_obj.GetAttachmentFromIndex(att);
			...
			if ( linked_scene_objects.Count() > 0 )
			{
				...
				CfgParamArray cfg_param_lnk = GetParamByName( param_name_lnk, CFG_TYPE_ARRAY );
				cfg_param_lnk.Clear();
		...
		CfgParamInt cfg_obj_count = GetParamByName(PARAM_OBJ_COUNT, CFG_TYPE_INT);
...
PrintString("Loading scene: "+ scene_name +" start");	
...
PrintString("LoadScene - loading Player finished..");
		
		// Load Scene Objects
		LoadSceneObjects(scene);
PrintString("LoadScene - loading scene objects finished..");
	...
	//=============================================
	// LOAD: LoadScenePlayer (position, rotation, damage, script)
	//=============================================
	void LoadScenePlayer(SceneData scene)
	{
		...
		{
			CfgParamArray 	cfg_player 		= GetParamByName(PARAM_PLAYER, CFG_TYPE_ARRAY);
			CfgParamString	cfg_ply_pos		= cfg_player.GetValueByName(PARAM_OBJ_POS, CFG_TYPE_STRING);
			CfgParamFloat	cfg_ply_rot		= cfg_player.GetValueByName(PARAM_OBJ_ROT, CFG_TYPE_FLOAT);
			CfgParamFloat	cfg_ply_dmg		= cfg_player.GetValueByName(PARAM_OBJ_DMG, CFG_TYPE_FLOAT);
			CfgParamString	cfg_ply_iscr	= cfg_player.GetValueByName(PARAM_OBJ_ISCR, CFG_TYPE_STRING);
			...
			// Set Damage
			scn_player.SetDamage(cfg_ply_dmg.GetValue());
	...
	//=============================================
	// LOAD: LoadSceneObjects (position, rotation, damage, script, scripted links)
	//=============================================
	void LoadSceneObjects(SceneData scene)
	{
		...
		for ( int i = 0; i < obj_count; ++i )
		{
			...
			CfgParamArray cfg_obj = GetParamByName(param_name, CFG_TYPE_ARRAY);
			
			CfgParamString	cfg_obj_type	= cfg_obj.GetValueByName(PARAM_OBJ_TYPE, CFG_TYPE_STRING);
			CfgParamString	cfg_obj_pos		= cfg_obj.GetValueByName(PARAM_OBJ_POS, CFG_TYPE_STRING);
			CfgParamFloat	cfg_obj_rot		= cfg_obj.GetValueByName(PARAM_OBJ_ROT, CFG_TYPE_FLOAT);
			CfgParamString	cfg_obj_ori		= cfg_obj.GetValueByName(PARAM_OBJ_ORI, CFG_TYPE_STRING);
			CfgParamFloat	cfg_obj_dmg		= cfg_obj.GetValueByName(PARAM_OBJ_DMG, CFG_TYPE_FLOAT);
			CfgParamString	cfg_obj_iscr	= cfg_obj.GetValueByName(PARAM_OBJ_ISCR, CFG_TYPE_STRING);
...
//PrintString(itoa(i)+ " Loading param_name: "+ param_name+ " => object type: "+ cfg_obj_type.GetValue());
			...
			if ( cfg_obj_ori.GetValue() != "" )
			{
				scn_obj.SetOrientation(cfg_obj_ori.GetValue().ToVector());
			}
			
			// Setup damage
			scn_obj.SetDamage(cfg_obj_dmg.GetValue());
			// Setup init script
			scn_obj.SetInitScript( DecodeInitScript(cfg_obj_iscr.GetValue()) );
			
			// Setup indices for linked objects
			CfgParamArray cfg_obj_param_links = GetParamByName(param_name + "_" + PARAM_OBJ_LNK, CFG_TYPE_ARRAY);
			array<CfgParam> cfg_obj_links = cfg_obj_param_links.GetValues();
			
			for ( int lnko = 0; lnko < cfg_obj_links.Count(); lnko++ )
			{
				CfgParamInt cfg_obj_param_lnk_int = cfg_obj_links.Get(lnko);
			...
			{				
				...
				array<CfgParam> cfg_param_attachments = cfg_param_att.GetValues();
				
				for ( int j = 0; j < cfg_param_attachments.Count(); ++j )
				{
					CfgParamString cfg_param_att_str = cfg_param_attachments.Get(j);
					scn_obj.GetObject().CreateAttachment( cfg_param_att_str.GetValue() );
				}
			}
		}
	}
