class PluginSceneManager extends PluginBase
{
	...
	{	
		...
		// Raycast from
		vector from = m_ModuleDeveloper.GetFreeCamera().GetPosition(); 
		// Raycast to
...
Print("Raycast obj: "+ obj.GetType());				
	...
	SceneObject Event_OnDoubleClick()
	{
		...
		return CreateSelectedObjectAtPointer();
	}
	
	//==========================================
	// CreateSelectedObjectAtPointer
	//==========================================
	SceneObject CreateSelectedObjectAtPointer()
	{
		if ( m_ClassBrowserSelected == "" )
		{
			SceneEditorMenuCmdPopup("Selecione uma classe antes de spawnar.");
			return NULL;
		}	
		
		vector contact_pos = GetMousePositionInScene();
		
		if ( GetGame().IsClient() && GetGame().IsMultiplayer() )
		{
			autoptr Param5<int, string, vector, vector, EntityAI> params = new Param5<int, string, vector, vector, EntityAI>(0, m_ClassBrowserSelected, contact_pos, "0 0 0", NULL);
			GetGame().GetPlayer().RPCSingleParam(DEV_RPC_CREATE_SCENE_OBJECT, params);
			SceneEditorMenuCmdPopup("Pedido de spawn enviado: " + m_ClassBrowserSelected);
			return NULL;
		}
		...
		SceneEditorMenuCmdPopup("Nao foi possivel criar: " + m_ClassBrowserSelected);
		EditorUpdate();
		return NULL;
	}

	//==========================================
	// RegisterSceneObjectFromServer
	//==========================================
	void RegisterSceneObjectFromServer(string class_name, vector pos, vector orientation, EntityAI entity)
	{
		if ( entity == NULL )
		{
			SceneEditorMenuCmdPopup("Servidor nao retornou objeto: " + class_name);
			return;
		}
		...
		SceneObject obj = m_SceneData.GetSceneObjectByEntityAI(entity);
		
		if ( obj == NULL )
		{
			obj = m_SceneData.AddObject(entity);
		}
		
		if ( obj )
		{
			SelectObject(obj);
			SceneEditorMenuCmdPopup("Spawn confirmado: " + class_name);
		}
	...
	{		
		...
		if ( GetGame().IsClient() && GetGame().IsMultiplayer() )
		{
			string scene_name = m_ModuleConfigDebugProfile.GetSceneEditorName();
			
			SceneUnload();
			m_SceneData = new SceneData;
			m_SceneData.SetNameScene(scene_name);
			return;
		}
...
//PrintString("Load link: "+ ToString(scene_object.GetObject()) +" <-> "+ ToString(scene_objects.Get(index).GetObject()) );
	...
	void DeleteSelectedObject()
	{
		...
		if ( selected_obj != NULL )
		{
			if ( !selected_obj.IsPlayer() )
			{
				...
				selected_obj.RequestDeleteOnServer();
	...
	void SelectedObjectDuplicate()
	{
		...
		if ( selected_obj )
		{
			...
			if ( GetGame().IsClient() && GetGame().IsMultiplayer() )
			{
				autoptr Param5<int, string, vector, vector, EntityAI> params = new Param5<int, string, vector, vector, EntityAI>(0, selected_obj.GetTypeName(), new_pos, selected_obj.GetOrientation(), NULL);
				GetGame().GetPlayer().RPCSingleParam(DEV_RPC_CREATE_SCENE_OBJECT, params);
				SceneEditorMenuCmdPopup("Pedido de duplicacao enviado: " + selected_obj.GetTypeName());
				return;
			}
			
			SceneObject obj = m_SceneData.CreateSceneObject(selected_obj.GetTypeName(), new_pos );
			obj.SetOrientation(selected_obj.GetOrientation());
	...
	//==========================================
	// SelectedObjectSetPitch
	//==========================================
	void SelectedObjectSetPitch(float pitch)
	{
		if ( GetSelectedSceneObject() )
		{
			GetSelectedSceneObject().SetPitch(pitch);
	...
	//==========================================
	// SelectedObjectSetRoll
	//==========================================
	void SelectedObjectSetRoll(float roll)
	{
		if ( GetSelectedSceneObject() )
		{
			GetSelectedSceneObject().SetRoll(roll);
	...
	//==========================================
	// SelectedObjectAddPosition
	//==========================================
	void SelectedObjectAddPosition(float add_x, float add_y, float add_z)
	{
		SceneObject selected_obj = GetSelectedSceneObject();
		...
		if ( selected_obj )
		{
			...
			v[1] = v[1] + add_y;
			v[2] = v[2] + add_z;
			selected_obj.SetPositionDirect(v);
	...
	//==========================================
	// SelectedObjectAddRotationAxis
	//==========================================
	void SelectedObjectAddRotationAxis(int axis, float add_rot)
	{
		if ( GetSelectedSceneObject() )
		{
			GetSelectedSceneObject().AddRotationAxis(axis, add_rot);
	...
	void SelectedObjectFocus()
	{
		...
		if ( selected_obj )
		{
			Camera		cam = m_ModuleDeveloper.GetFreeCamera();
...
Print("SceneManager - Editor update finished...");
	...
	void ExecuteEnforceScript( int index, SceneObject scene_object, string script )
	{
		...
		EntityAI obj = scene_object.GetObject();
	...
	protected bool IsMouseInScene()
	{
		...
		if ( widget_under_mouse != NULL && ( widget_under_mouse.GetName() == "pnl_SceneControl" || widget_under_mouse.GetName() == "SceneEditorShade" ) )
		{
			return true;
		}
	...
	{
		vector dir = GetGame().GetPointerDirection();
		Camera free_cam = m_ModuleDeveloper.GetFreeCamera();
		if ( free_cam == NULL )
		{
		...
		vector from = free_cam.GetPosition(); 
		vector to = from + ( dir * 10000 );   
		vector contact_pos;
		vector contact_dir;
		int contact_component;
