...
{
	autoptr multiMap<int, string> 	m_NotifiersLabel;
	autoptr multiMap<int, int>		m_NotifiersIndexColor;
	autoptr multiMap<int, int>		m_NotifiersLevel;

	private autoptr multiMap<int, string>	m_NotifiersIcons;
	...
	void PluginPlayerStatus()
	{
		...
		m_NotifiersLevel = new multiMap<int, int>; // [key] level
	...
	void ~PluginPlayerStatus()
	{
		delete m_NotifiersLabel;
		delete m_NotifiersIndexColor;
		delete m_NotifiersLevel;
		delete m_NotifiersIcons;
	}
	...
	void SetNotifier( int key, int index = 9, string label = "", int color =  0xFFFFFFFF )
	{
		if ( key )
		{
			...
			if ( mission )
			{
				...
				if ( hud )
				{
					hud.DisplayStatusLabel( key, label );
				}
			}
		}
	}

	void DisplayTendency( int key, int tendency )
	{
		if ( key )
		{
			// display icon
			...
			if ( m_NotifiersIcons.HasKey( key ) )
			{
				...
				if ( mission )
				{
					...
					if ( hud )
					{
						hud.DisplayNotifier( key, tendency );
					}
				}
			}
		}
	...
	void DisplayStatusLevel( int key, int level )
	{
		if ( key )
		{
			if ( m_NotifiersLevel.HasKey( key ) )
			{
				m_NotifiersLevel.Remove( key );
			}
			m_NotifiersLevel.Insert( key, level );
			...
			if ( mission )
			{
				...
				if ( hud )
				{
					hud.DisplayStatusLevel( key, level );
				}
			}
		}
	}
