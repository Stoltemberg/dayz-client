class Bleeding: ModifierBase
{
	...
	private void OnActivate(PlayerBase player)
	{
		...
		if( player.GetDisplayStatus() ) player.GetDisplayStatus().SetStatus(DELM_BADGE_BLEEDING, true);
	}
	...
	private void OnDeactivate(PlayerBase player)
	{
		...
		if( player.GetDisplayStatus() ) player.GetDisplayStatus().SetStatus(DELM_BADGE_BLEEDING, false);
	}
