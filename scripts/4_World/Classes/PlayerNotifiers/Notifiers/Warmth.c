class Warmth: NotifierBase
{
	...
	void DisplayTendency(float delta)
	{
		...
		GetDisplayStatus().SetStatus(DELM_TDCY_TEMPERATURE,tendency);
	}
	...
	private void DisplayBadge()
	{
		...
		{
			GetDisplayStatus().SetStatus(DELM_NTFR_TEMPERATURE,DELM_LVL_1);
		}
		else if ( temperature >= HOT_TRESHOLD )
		{
			GetDisplayStatus().SetStatus(DELM_NTFR_TEMPERATURE,DELM_LVL_2);
		}
		else if ( temperature >= OVERHEATING_TRESHOLD )
		{
			GetDisplayStatus().SetStatus(DELM_NTFR_TEMPERATURE,DELM_LVL_3);
		}
		else if ( temperature <= HYPOTHERMIA_TRESHOLD )
		{
			GetDisplayStatus().SetStatus(DELM_NTFR_TEMPERATURE,DELM_LVL_6);
		}
		else if ( temperature <= FREEZING_TRESHOLD )
		{
			GetDisplayStatus().SetStatus(DELM_NTFR_TEMPERATURE,DELM_LVL_5);
		}
		else if ( temperature <= COLD_TRESHOLD )
		{
			GetDisplayStatus().SetStatus(DELM_NTFR_TEMPERATURE,DELM_LVL_4);
		}
		else
		{
			GetDisplayStatus().SetStatus(DELM_NTFR_TEMPERATURE,DELM_LVL_0);
		}
	}
