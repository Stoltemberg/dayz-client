class ChatLine
{
	...
	{
		m_text_widget_bg = GetGame().GetWorkspace().CreateWidgets("gui/layouts/day_z_chat_item.layout", root_widget);
		m_text_widget_bg.Show(false);
		m_text_widget_bg.SetColor(0xFF000000);
		
		m_text_widget = GetGame().GetWorkspace().CreateWidgets("gui/layouts/day_z_chat_item.layout", root_widget);
	...
	void ~ChatLine()
	{
		if (m_text_widget) m_text_widget.Destroy();
		if (m_text_widget_bg) m_text_widget_bg.Destroy();
	}
	...
	void Set(ChatMessageEventParams params)
	{
		...
		autoptr TFloatArray color = new TFloatArray;
		...
		if (params.param1 != CCStatus || params.param4 == "")
		{
			...
			switch(params.param1)
			{
				...
				break;
				case CCVehicle:
					GetGame().ConfigGetFloatArray("RscChatListDefault colorVehicleChannel", color);
				break;
				case CCDirect:
