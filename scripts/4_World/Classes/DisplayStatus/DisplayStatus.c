class DisplayStatus
{
	...
	void DisplayStatus(PlayerBase player)
	{
		...
		AddElement(DELM_BADGE_BLEEDING, new BadgeBleeding);// size 1
		...
		// sum 29/31
		AddElement(DELM_NTFR_TEMPERATURE, new InvBadgeTemperature);// size 3
		AddElement(DELM_NTFR_FEVER, new InvBadgeFever);// size 3
		AddElement(DELM_NTFR_FRACTURE, new InvBadgeFracture);// size 3
		AddElement(DELM_NTFR_HEALTHY, new InvBadgeLives);// size 3
