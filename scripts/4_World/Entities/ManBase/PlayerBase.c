...
{	
	...
	{		
		...
		m_ModulePlayerStatus	= (PluginPlayerStatus)GetPlugin(PluginPlayerStatus);
	...
	void OnRPC(int rpc_type, ParamsReadContext ctx)
	{
		...
		if ( rpc_type == ADMIN_RPC_AUTH_REQUEST || rpc_type == ADMIN_RPC_AUTH_RESPONSE || rpc_type == ADMIN_RPC_PLAYER_LIST_REQUEST || rpc_type == ADMIN_RPC_PLAYER_LIST_RESPONSE || rpc_type == ADMIN_RPC_COMMAND || rpc_type == ADMIN_RPC_MESSAGE )
		{
			PluginAdmin module_admin = GetPlugin(PluginAdmin);
			if ( module_admin )
			{
				module_admin.OnRPC(this, rpc_type, ctx);
			}
			return;
		}

		if ( rpc_type == CHAT_RPC_SEND || rpc_type == CHAT_RPC_DELIVER )
		{
			PluginCustomChat module_chat = GetPlugin(PluginCustomChat);
			if ( module_chat )
			{
				module_chat.OnRPC(this, rpc_type, ctx);
			}
			return;
		}

		if ( rpc_type == PARTY_RPC_COMMAND || rpc_type == PARTY_RPC_STATE || rpc_type == PARTY_RPC_MESSAGE )
		{
			PluginParty module_party = GetPlugin(PluginParty);
			if ( module_party )
			{
				module_party.OnRPC(this, rpc_type, ctx);
			}
			return;
		}

		if ( rpc_type == SPAWN_SELECTOR_RPC_SHOW || rpc_type == SPAWN_SELECTOR_RPC_SELECT || rpc_type == SPAWN_SELECTOR_RPC_MESSAGE )
		{
			PluginSpawnSelector module_spawn_selector = GetPlugin(PluginSpawnSelector);
			if ( module_spawn_selector )
			{
				module_spawn_selector.OnRPC(this, rpc_type, ctx);
			}
			return;
		}
		...
		else if ( rpc_type == DEV_RPC_SPAWN_ITEM || rpc_type == DEV_RPC_CLEAR_INV || rpc_type == DEV_RPC_TELEPORT || rpc_type == DEV_RPC_SET_PLAYER_DIRECTION || rpc_type == DEV_RPC_SEND_SERVER_LOG || rpc_type == DEV_RPC_TOGGLE_GOD_MODE || rpc_type == DEV_RPC_DEBUG_PING || rpc_type == DEV_RPC_CREATE_SCENE_OBJECT || rpc_type == RPC_SYNC_SCENE_OBJECT )
		{
			PluginDeveloper module_rc_debug = GetPlugin(PluginDeveloper);
			if ( module_rc_debug )
			{
				module_rc_debug.OnRPC(this, rpc_type, ctx);
			}
		}
	}
