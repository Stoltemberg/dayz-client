...
class MissionBase extends Mission
{
	...
	UIScriptedMenu CreateScriptedMenu(int id)
	{
		...
		case MENU_PARTY:
			menu = new PartyMenu;
			break;
		case MENU_SPAWN_SELECTOR:
			menu = new SpawnSelectorMenu;
			break;
		}
	...
	void OnKeyPress(int key)
	{
		...
		{
			PluginKeyBinding plugin = (PluginKeyBinding)GetPlugin(PluginKeyBinding);
			if ( plugin )
			{
	...
	bool ToggleSceneEditorMenu()
	{
		PluginSceneManager scene_manager = (PluginSceneManager)GetPluginManager().GetPluginByType(PluginSceneManager);

		if ( scene_manager )
		{
			Print("[SceneEditor] Toggle pelo atalho P.");
			scene_manager.EditorToggle();
			return true;
		}

		Print("[SceneEditor] PluginSceneManager nao registrado.");
		return false;
	}
	...
	bool ToggleCustomMapMenu()
	{
		...
		{
			Print("[CustomMap] Fechando MENU_MAP pelo atalho M.");
			manager.Back();
			return true;
		}
		...
		if ( current_menu )
		{
			...
			{
				Print("[CustomMap] Atalho M ignorado: outro menu aberto.");
				return false;
			}
		...
		Print("[CustomMap] Abrindo MENU_MAP pelo atalho M.");
	...
	void OnMouseButtonPress(int button)
	{
		...
		if ( IsPluginManagerExists() )
		{
			...
			if ( plugin )
			{
				plugin.OnMouseButtonPress(button);
			}
		}
	}
	...
	void OnMouseButtonRelease(int button)
	{
		...
		if ( IsPluginManagerExists() )
		{
			...
			if ( plugin )
			{
				plugin.OnMouseButtonRelease(button);
			}
		}
	}
