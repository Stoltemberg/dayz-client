...
class MissionGameplay extends MissionBase
{
	...
	int		m_LastMouseInputWatchdogTime;
	...
	{	
		m_initialized = false;
		m_LastMouseInputWatchdogTime = 0;
	...
	void OnInit()
	{
		...
		if ( GetGame().IsDebug() || GetPlugin(PluginConfigDebugProfile) )
		{
	...
	{	
		...
		if ( m_LastMouseInputWatchdogTime > 0 && (time_now - m_LastMouseInputWatchdogTime) > 1500 )
		{
			ClearStuckMouseInput();
		}
		m_LastMouseInputWatchdogTime = time_now;
		...
		if ( m_hud )
		{
			m_hud.UpdateCustomMapHudMarkers();
		}
	...
	{
		HudDebug hud_debug = GetHudDebug();
		if ( hud_debug )
		{
			hud_debug.RefreshCrosshairVisibility();
		}
	}
