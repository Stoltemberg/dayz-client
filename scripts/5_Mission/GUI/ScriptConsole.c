...
class ScriptConsole extends UIScriptedMenu
{
	...
	Widget Init()
	{
		...
		m_DebugGodMode		= layoutRoot.FindAnyWidget("cbx_DebugGodMode");
		m_DebugFreeCamera	= layoutRoot.FindAnyWidget("cbx_DebugFreeCamera");
		m_DebugTeleportCursor = layoutRoot.FindAnyWidget("cbx_DebugTeleportCursor");
		m_DebugSpawnClipboard = layoutRoot.FindAnyWidget("cbx_DebugSpawnClipboard");
		m_HudTests = new HudDebugHudTests( m_Tabs[TABS_GENERAL] );
		...
		RefreshDeveloperActionCheckboxes();
	...
	void RunEnscript()
	{
		...
		if ( !GetGame().IsDebug() )
		{
			autoptr Param1<string> msg = new Param1<string>("[EnfScript] ExecuteEnforceScript não é permitido neste executável release. Use a aba SQF, ou um executável interno/debug do DayZ se existir nesta build.");
			CallMethod(CALL_ID_SCR_CNSL_ADD_PRINT, msg);
	...
	void RefreshHudDebugByLocalProfile()
	{
		if ( m_MissionGameplay )
		{
			HudDebug hud_debug = m_MissionGameplay.GetHudDebug();
			if ( hud_debug )
			{
				hud_debug.RefreshByLocalProfile();
			}
		}
	}

	void RefreshDeveloperActionCheckboxes()
	{
		if ( m_Developer )
		{
			if ( m_DebugGodMode )
			{
				m_DebugGodMode.SetChecked(m_Developer.IsGodMode());
			}
			
			if ( m_DebugFreeCamera )
			{
				m_DebugFreeCamera.SetChecked(m_Developer.IsEnabledFreeCamera());
			}
		}
		
		if ( m_DebugTeleportCursor )
		{
			m_DebugTeleportCursor.SetChecked(false);
		}
		
		if ( m_DebugSpawnClipboard )
		{
			m_DebugSpawnClipboard.SetChecked(false);
		}
	}
	...
	bool OnClick(Widget w, int x, int y, int button)
	{
		...
		else if ( w == m_DebugGodMode )
		{
			if ( m_Developer )
			{
				m_Developer.ToggleGodModeThisPlayer();
				m_DebugGodMode.SetChecked(m_Developer.IsGodMode());
			}
			return true;
		}
		else if ( w == m_DebugFreeCamera )
		{
			if ( m_Developer )
			{
				m_Developer.ToggleFreeCamera();
				m_DebugFreeCamera.SetChecked(m_Developer.IsEnabledFreeCamera());
				GetGame().GetCallQueue(CALL_CATEGORY_SYSTEM).Call(GetGame().GetMission(), "RefreshCrosshairVisibility", NULL);
			}
			return true;
		}
		else if ( w == m_DebugTeleportCursor )
		{
			if ( m_Developer )
			{
				m_Developer.TeleportAtCursor();
			}
			m_DebugTeleportCursor.SetChecked(false);
			return true;
		}
		else if ( w == m_DebugSpawnClipboard )
		{
			if ( m_Developer )
			{
				m_Developer.SpawnFromClipboard();
			}
			m_DebugSpawnClipboard.SetChecked(false);
			return true;
		}
		...
		else if ( w == m_ObjectsTextListbox )
		{
			...
			HideItemButtons();
			ShowItemTransferButtons();
			m_SelectedObjectText.SetText( "Objeto: " + GetCurrentObjectName() );
		...
		else if ( w == m_PresetItemsTextListbox )
		{
			...
			ShowItemButtons();
			ShowItemTransferButtons();
			m_SelectedObjectText.SetText( "Objeto: " + GetCurrentItemName() );
	...
	CheckBoxWidget		m_DebugGodMode;
	CheckBoxWidget		m_DebugFreeCamera;
	CheckBoxWidget		m_DebugTeleportCursor;
	CheckBoxWidget		m_DebugSpawnClipboard;
	HudDebugHudTests	m_HudTests;
